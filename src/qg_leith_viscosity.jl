...
@inline ∂yb_times_f2_div_N2(i, j, k, grid, coriolis, buoyancy, tracers) = ℑxyzᶜᶜᶠ(i, j, k, grid, fᶠᶠᵃ, coriolis) / 
                                                                          max(0, ∂z_b(i, j, k, grid, buoyancy, tracers)) *
...
@inline function abs²_∇h_q(i, j, k, grid, coriolis, buoyancy, tracers)
    ...
    ∂zqx = min(∂zᶜᶜᶜ(i, j, k, grid, ∂xb_times_f2_div_N2, coriolis, buoyancy, tracers), 1e-4)
    ∂zqy = min(∂zᶜᶜᶜ(i, j, k, grid, ∂yb_times_f2_div_N2, coriolis, buoyancy, tracers), 1e-4)

    return (∂zqx^2 + ∂zqy^2)
end

"Return the filter width for a Leith Diffusivity on a general grid."
@inline Δᶜᶜᶜ(i, j, k, grid) =  2 * (1 / (1 / Δxᶜᶜᶜ(i, j, k, grid)^2 + 1 / Δyᶜᶜᶜ(i, j, k, grid)^2))
...
@kernel function calculate_qgleith_viscosity!(ν, grid, closure, coriolis, buoyancy, velocities, tracers)
    ...
    @inbounds ν[i, j, k] = (C * Δᶜᶜᶜ(i, j, k, grid) / π)^3 * sqrt(∂ζ² + ∂δ² + ∂q²) 
end
