...
function plot_stuff_all_three!(ax, E4, E8, E16, E50)
    ...
    if length(D4) > 2
        ...
        scatter!(ax, days2, D8[3], markersize  = 12.5, marker = :cross, color = Symbol("red"))
        scatter!(ax, days2, D16[3], markersize = 15,   marker = :cross, color = Symbol("red"))
    end
    if length(D4) > 3
...
function heatmap_yzplane(filename, variable; fig = nothing, 
    ...
    for i in 1:length(map) - 1
        if !isnothing(plot_titles)
            ax = Axis(fig[row, i]; title = plot_titles[i])
        else
            ax = Axis(fig[row, i])
        end
    ...
    if filename isa WhiteContour
        ...
        for i in eachindex(titles)
            if isnothing(variable)
                ...
                map[i] = file[variable * "/" * titles[i]][level]./ normalize_val
            end
        end
    end
...
function BTenergy_contours(fig)
    fig = heatmap_energies("energies_quarter"; fig, row = 1, mean_vol = mean_vol4,  colormap = :viridis, maxrange = 0.4, plot_titles = plot_titles_quarter)
    fig = heatmap_energies("energies_eight";   fig, row = 2, mean_vol = mean_vol8,  colormap = :viridis, maxrange = 0.8, plot_titles = plot_titles_eight  )
    fig = heatmap_energies("energies_sixteen"; fig, row = 3, mean_vol = mean_vol16, colormap = :viridis, maxrange = 0.8, plot_titles = plot_titles_sixteen)
    return fig
end

function energy_contours(fig)
    fig = heatmap_energies("contours_quarter", "energy"; level = 1, fig, row = 1, normalize_val = mean_vol4,  colormap = :viridis, maxrange = 0.4, plot_titles = plot_titles_quarter)
    fig = heatmap_energies("contours_eight", "energy";   level = 1, fig, row = 2, normalize_val = mean_vol8,  colormap = :viridis, maxrange = 0.8, plot_titles = plot_titles_eight  )
    fig = heatmap_energies("contours_sixteen", "energy"; level = 1, fig, row = 3, normalize_val = mean_vol16, colormap = :viridis, maxrange = 0.8, plot_titles = plot_titles_sixteen)
    return fig
end

function vorticity_contours(fig, level = 1)
    fig = heatmap_energies("contours_quarter", "vorticity"; fig, row = 1, level, colormap = Reverse(:balance), normalize_val = 1e-5, maxrange = 1, minrange = - 1, plot_titles = plot_titles_quarter)
    fig = heatmap_energies("contours_eight", "vorticity";   fig, row = 2, level, colormap = Reverse(:balance), normalize_val = 1e-5, maxrange = 1, minrange = - 1, plot_titles = plot_titles_eight  )
    fig = heatmap_energies("contours_sixteen", "vorticity"; fig, row = 3, level, colormap = Reverse(:balance), normalize_val = 1e-5, maxrange = 1, minrange = - 1, plot_titles = plot_titles_sixteen)
    return fig
end

function enstrophy_contours(fig, level = 1)
    fig = heatmap_energies("contours_quarter", "enstrophy"; fig, row = 1, level, colormap = :solar, maxrange = 0.5, normalize_val = 1e-10, plot_titles = plot_titles_quarter)
    fig = heatmap_energies("contours_eight", "enstrophy";   fig, row = 2, level, colormap = :solar, maxrange = 1.0, normalize_val = 1e-10, plot_titles = plot_titles_eight  )
    fig = heatmap_energies("contours_sixteen", "enstrophy"; fig, row = 3, level, colormap = :solar, maxrange = 2.0, normalize_val = 1e-10, plot_titles = plot_titles_sixteen)
    return fig
end
...
function yzbuoyancy_contours(fig, level = 1)
    fig = heatmap_yzplane("contours_quarter", "yzbuoyancy2"; fig, row = 1, hide_spines = false, level, colormap = :thermometer, maxrange = 0.006, minrange = -0.004, plot_titles = plot_titles_quarter)
    fig = heatmap_yzplane("contours_eight", "yzbuoyancy2";   fig, row = 1, hide_spines = false, level, colormap = :thermometer, maxrange = 0.006, minrange = -0.004, plot_titles = plot_titles_eight  )
    fig = heatmap_yzplane("contours_sixteen", "yzbuoyancy2"; fig, row = 1, hide_spines = false, level, colormap = :thermometer, maxrange = 0.006, minrange = -0.004, plot_titles = plot_titles_sixteen)
    return fig
end

function refpe_contours(fig, level = 1)
    fig = heatmap_energies("contours_quarter", "referencepe"; level, fig, row = 1, colormap = :thermometer, plot_titles = plot_titles_quarter)
    fig = heatmap_energies("contours_eight", "referencepe";   level, fig, row = 2, colormap = :thermometer, plot_titles = plot_titles_eight  )
    fig = heatmap_energies("contours_sixteen", "referencepe"; level, fig, row = 3, colormap = :thermometer, plot_titles = plot_titles_sixteen)
    return fig
end
