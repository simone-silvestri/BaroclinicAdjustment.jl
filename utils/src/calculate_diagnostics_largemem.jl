...
function compute_energy_diagnostics(f::Dict, iterations)
    ...
    CUDA.@allowscalar begin
    	EKE = Diagnostics.propagate(E, KEavg; func = (e, ē) -> e - ē, path = mypath * "auxiliaries/energies.jld2", name = "old_EKE")
    end
...
function compute_energy_timeseries(f)
    ū = propagate(f[:u]; func = x -> mean(x, dims = 1), path = mypath * "auxiliaries/mean.jld2", name = "ū")
    v̄ = propagate(f[:v]; func = x -> mean(x, dims = 1), path = mypath * "auxiliaries/mean.jld2", name = "v̄")
    b̄ = propagate(f[:b]; func = x -> mean(x, dims = 1), path = mypath * "auxiliaries/mean.jld2", name = "b̄")

    u′ = propagate(f[:u], ū; func = (x, X) -> x - X, path = mypath * "auxiliaries/variance.jld2", name = "u′")
    v′ = propagate(f[:v], v̄; func = (x, X) -> x - X, path = mypath * "auxiliaries/variance.jld2", name = "v′")
    b′ = propagate(f[:b], b̄; func = (x, X) -> x - X, path = mypath * "auxiliaries/variance.jld2", name = "b′")

    B = propagate(f[:b]; func = x -> mean(x, dims = 2))

    B̄  = propagate(b̄, B; func = (b̄, B) -> b̄ - B, path = mypath * "auxiliaries/energies.jld2", name = "B̄")
    N² = propagate(B; func = B -> StratificationOperation(B), path = mypath * "auxiliaries/energies.jld2", name = "N²")

    MEKE = propagate(ū , v̄ ; func = (u, v)  -> mean(0.5 * (u^2 + v^2)), path = mypath * "auxiliaries/energies.jld2", name = "MEKE")
    EKE  = propagate(u′, v′; func = (u, v)  -> mean(0.5 * (u^2 + v^2)), path = mypath * "auxiliaries/energies.jld2", name = "EKE")
    MAPE = propagate(B̄ , N²; func = (B, N²) -> mean(0.5 * B^2 / N²),    path = mypath * "auxiliaries/energies.jld2", name = "MAPE")
    EAPE = propagate(b′, N²; func = (b, N²) -> mean(0.5 * b^2 / N²),    path = mypath * "auxiliaries/energies.jld2", name = "EAPE")
...
function compute_variances(f::Dict, fm, iterations)
    CUDA.@allowscalar begin
        u′ = propagate(f[:u], fm.U; func = (x, X) -> x - X, path = mypath * "auxiliaries/new_variance.jld2", name = "u′")
        v′ = propagate(f[:v], fm.V; func = (x, X) -> x - X, path = mypath * "auxiliaries/new_variance.jld2", name = "v′")
        w′ = propagate(f[:w], fm.W; func = (x, X) -> x - X, path = mypath * "auxiliaries/new_variance.jld2", name = "w′")
        b′ = propagate(f[:b], fm.B; func = (x, X) -> x - X, path = mypath * "auxiliaries/new_variance.jld2", name = "b′")
    end
    
    u′² = time_average(propagate(u′; func = x -> x^2, path = mypath * "auxiliaries/new_variance.jld2", name = "u′²"), iterations)
    v′² = time_average(propagate(v′; func = x -> x^2, path = mypath * "auxiliaries/new_variance.jld2", name = "v′²"), iterations)
    w′² = time_average(propagate(w′; func = x -> x^2, path = mypath * "auxiliaries/new_variance.jld2", name = "w′²"), iterations)
    b′² = time_average(propagate(b′; func = x -> x^2, path = mypath * "auxiliaries/new_variance.jld2", name = "b′²"), iterations)

    u′v′ = time_average(propagate(u′, v′; func = (x, y) -> x * y, path = mypath * "auxiliaries/new_variance.jld2", name = "u′v′"), iterations)
    u′w′ = time_average(propagate(u′, w′; func = (x, y) -> x * y, path = mypath * "auxiliaries/new_variance.jld2", name = "u′w′"), iterations)
    v′w′ = time_average(propagate(v′, w′; func = (x, y) -> x * y, path = mypath * "auxiliaries/new_variance.jld2", name = "v′w′"), iterations)

    u′b′ = time_average(propagate(u′, b′; func = (x, y) -> x * y, path = mypath * "auxiliaries/new_variance.jld2", name = "u′b′"), iterations)
    v′b′ = time_average(propagate(v′, b′; func = (x, y) -> x * y, path = mypath * "auxiliaries/new_variance.jld2", name = "v′b′"), iterations)
    w′b′ = time_average(propagate(w′, b′; func = (x, y) -> x * y, path = mypath * "auxiliaries/new_variance.jld2", name = "w′b′"), iterations)
...
function compute_instability(fields, fm, iterations)
    ...
    CUDA.@allowscalar begin
        b′ = propagate(fields[:b], fm.b̄; func = (x, X) -> x - X, path = mypath * "auxiliaries/new_new_variance.jld2", name = "b′")
        u′ = propagate(fields[:u], fm.ū; func = (x, X) -> x - X, path = mypath * "auxiliaries/new_new_variance.jld2", name = "u′")
        v′ = propagate(fields[:v], fm.v̄; func = (x, X) -> x - X, path = mypath * "auxiliaries/new_new_variance.jld2", name = "v′")
    end
    
    u′b′ = time_average(propagate(u′, b′; func = (x, y) -> x * y, path = mypath * "auxiliaries/new_new_variance.jld2", name = "u′b′"), iterations)
    v′b′ = time_average(propagate(v′, b′; func = (x, y) -> x * y, path = mypath * "auxiliaries/new_new_variance.jld2", name = "v′b′"), iterations)
...
function calculate_diagnostics(trailing_character = "_weaker", file_prefix = generate_names())
    ...
    for (prefix, filename) in zip(file_prefix, filenames)
	    ...
	    postprocess[:instab]    = move_on_cpu(instab)
            ...
            try run(`mv $(mypath)/auxiliaries/mean.jld2 $(mypath)/auxiliaries/$(file_prefix)_mean.jld2`); catch; end
            try run(`mv $(mypath)/auxiliaries/variance.jld2 $(mypath)/auxiliaries/$(file_prefix)_variance.jld2`); catch; end
            try run(`mv $(mypath)/auxiliaries/energies.jld2 $(mypath)/auxiliaries/$(file_prefix)_energies.jld2`); catch; end
            try run(`mv $(mypath)/auxiliaries/new_variance.jld2 $(mypath)/auxiliaries/$(file_prefix)_new_variance.jld2`); catch; end
            try run(`mv $(mypath)/auxiliaries/new_new_variance.jld2 $(mypath)/auxiliaries/$(file_prefix)_new_new_variance.jld2`); catch; end
        end
    end
